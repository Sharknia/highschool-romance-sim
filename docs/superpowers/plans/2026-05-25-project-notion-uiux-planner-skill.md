...
## Iteration: Reproduce the Previous UX/UI Goal

After the first implementation, the user clarified that the skill must reproduce the previous UX/UI planning goal and later instructions, not only create a generic Notion planning outline.

Additional acceptance criteria:

- The skill must explicitly choose between idea-to-plan and UI/UX audit-to-plan modes.
- The audit mode must tell Codex to run or inspect the web app, capture desktop/mobile evidence, map screenshots into the Notion document, and distinguish mock checks from real execution.
- The workflow must require a short plan before writing the Notion document.
- The workflow must require a completion review gate and prohibit final reporting until the plan is complete or blocked/partial status is explicit.
- The final report must summarize what the Notion plan actually says, not only provide links or file paths.
- The skill must be forward-tested with a subagent before and after the update.
