...
- goal을 완료했을 때에는 사용 가능한 환경에서 반드시 `send-report` 스킬로 완료 보고를 전송한다.
- 현재 세션에 `send-report` 스킬이 없으면 실행하지 않고, 최종 응답에 미실행 사유와 완료 요약을 남긴다.
...
- push는 사용자가 명시적으로 요청했거나, 해당 원격 작업 서버의 운영 정책상 요구되는 경우에만 태스크/서브태스크 단위로 수행한다.
