...
const projectIdHelperPath = "apps/web/src/client/pages/projects/projectId.ts";
...
assert.ok(existsSync(join(root, projectIdHelperPath)), "클라이언트 projectId slug 규칙은 공용 helper로 분리해야 합니다.");
...
[
  ...
  "StudioGenerationAssistDrawer",
  "GenerationDiagnosticsSurface",
  "renderGenerationAssistDrawer",
  "generationAssistMode",
  "generationAssistSourceType",
  "actualPatchBadge",
  "mockReplayBadge",
  "protocolReplayBadge",
  "unavailableGenerationBadge",
  "rawGenerationDiagnostics",
  "data-generation-assist-surface",
  "data-generation-source-type",
  "생성 보조는 선택형 drawer",
  "수동 제작이 기본 흐름",
  "Problems Panel validation issue flow"
].forEach((requiredText) => {
  const pattern = new RegExp(requiredText.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"));
  assert.match(studioWorkspaceSource, pattern, `StudioWorkspace 생성 보조 drawer/진단 분리에 '${requiredText}' 흐름이 있어야 합니다.`);
});
...
assert.match(projectNewPageSource, /from "\.\/projectId"/, "ProjectNewPage는 공용 projectId slug helper를 사용해야 합니다.");
...
assert.match(heroineDetailSource, /setHeroineSourceProjectDirectory\(result\.projectDirectory \|\| ""\)/, "히로인 상세는 원본 라이브러리 저장 위치를 유지해야 합니다.");
assert.match(heroineDetailSource, /createProjectFromHeroine\(postAuthedJson,\s*heroine,\s*heroineSourceProjectDirectory\)/, "히로인 상세의 프로젝트 생성은 원본 라이브러리 디렉터리를 전달해야 합니다.");
...
const heroineApiSource = readText("apps/web/src/client/pages/heroines/heroineApi.ts");
...
assert.match(heroineApiSource, /from "\.\.\/projects\/projectId"/, "히로인 API는 공용 projectId slug helper를 사용해야 합니다.");
assert.match(heroineApiSource, /sourceProjectDirectory:\s*sourceProjectDirectory \|\| undefined/, "히로인 상세 프로젝트 생성 API는 원본 라이브러리 디렉터리를 서버에 전달해야 합니다.");
assert.match(heroineApiSource, /projectDirectory:\s*`workspace\/\$\{projectId\}\.vnmaker`/, "히로인 상세 프로젝트 생성 API는 라이브러리 기본 workspace를 덮어쓰지 않고 새 프로젝트 디렉터리를 써야 합니다.");
assert.match(heroineApiSource, /projectId,/, "히로인 상세 프로젝트 생성 API는 새 프로젝트 id를 명시해야 합니다.");
assert.match(heroineApiSource, /suggestProjectId\(title,\s*heroine\.id\)/, "히로인 상세 프로젝트 생성 API는 한글 이름 slug가 비어도 고유 heroine.id를 프로젝트 id fallback으로 써야 합니다.");
assert.doesNotMatch(heroineApiSource, /\|\|\s*"new-project"/, "히로인 상세 프로젝트 생성 API는 모든 한글 히로인에 같은 new-project fallback을 쓰면 안 됩니다.");
assert.match(heroineApiSource, /title,/, "히로인 상세 프로젝트 생성 API는 새 프로젝트 title을 명시해야 합니다.");
assert.match(heroineApiSource, /premise/, "히로인 상세 프로젝트 생성 API는 새 프로젝트 premise를 명시해야 합니다.");

const tempRoot = await mkdtemp(join(tmpdir(), "vn-maker-alpha-shell-"));
try {
  const bundledHeroineApiPath = join(tempRoot, "heroine-api.mjs");
  await esbuild({
    entryPoints: ["apps/web/src/client/pages/heroines/heroineApi.ts"],
    bundle: true,
    format: "esm",
    platform: "node",
    outfile: bundledHeroineApiPath
  });
  const { createProjectFromHeroine } = await import(pathToFileURL(bundledHeroineApiPath).href);
  let capturedProjectRequest = null;
  await createProjectFromHeroine((path, body) => {
    capturedProjectRequest = { path, body };
    return Promise.resolve({ ok: true });
  }, {
    id: "sakura_01",
    name: "사쿠라",
    description: "한국어 이름 케이스",
    personality: "차분함",
    speechStyle: "부드러움",
    appearance: "긴 머리"
  }, "/tmp/source-library.vnmaker");
  assert.equal(capturedProjectRequest.path, "/api/projects/from-heroine");
  assert.equal(capturedProjectRequest.body.sourceProjectDirectory, "/tmp/source-library.vnmaker");
